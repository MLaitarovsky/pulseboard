...
router.post('/:teamId/annotations', async (req: Request, res: Response) => {
  try {
    ...
    try {
      await redis.publish(`team:${teamId}:annotations`, JSON.stringify({
        type: 'new_annotation',
        annotation,
      }));
    } catch (e) {
    ...
    console.log(`📌 Annotation added: "${content.trim().substring(0, 40)}..." at ${timestampTarget}`);
...
router.delete('/:teamId/annotations/:annotationId', async (req: Request, res: Response) => {
  try {
    const teamId = await resolveTeamId(req.params.teamId);
    if (!teamId) return res.status(404).json({ error: 'Team not found' });
