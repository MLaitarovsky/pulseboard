...
router.get('/:teamId/incidents/:incidentId', async (req: Request, res: Response) => {
  try {
    const teamId = await resolveTeamId(req.params.teamId);
    if (!teamId) return res.status(404).json({ error: 'Team not found' });
...
router.post('/:teamId/incidents', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [teamId, title, description || '', severity || 'medium', createdBy || 'system']);
...
router.patch('/:teamId/incidents/:incidentId', async (req: Request, res: Response) => {
  try {
    const teamId = await resolveTeamId(req.params.teamId);
    if (!teamId) return res.status(404).json({ error: 'Team not found' });
    ...
    if (status === 'resolved') {
      updateQuery = 'UPDATE incidents SET status = $1, updated_at = NOW(), resolved_at = NOW() WHERE id = $2';
    } else if (status === 'reopened') {
      updateQuery = 'UPDATE incidents SET status = $1, updated_at = NOW(), resolved_at = NULL WHERE id = $2';
    } else {
      updateQuery = 'UPDATE incidents SET status = $1, updated_at = NOW() WHERE id = $2';
    }
    ...
    await pool.query(`
      INSERT INTO incident_timeline (incident_id, action, actor, message)
      VALUES ($1, $2, $3, $4)
    `, [incidentId, status, actor || 'demo-user', message || `Status changed to ${status}`]);

    const updated = await pool.query('SELECT * FROM incidents WHERE id = $1', [incidentId]);
...
router.patch('/:teamId/incidents/:incidentId/severity', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!severity || !VALID_SEVERITIES.includes(severity)) {
      client.release();
      return res.status(400).json({ error: `Invalid severity "${severity}". Valid: ${VALID_SEVERITIES.join(', ')}` });
    }
    ...
    if (oldSeverity === severity) {
      client.release();
      return res.status(400).json({ error: `Severity is already ${severity}` });
    }

    if (incident.status === 'resolved') {
      client.release();
      return res.status(400).json({ error: 'Cannot change severity of a resolved incident. Reopen it first.' });
    }
    ...
    const updated = await client.query('SELECT * FROM incidents WHERE id = $1', [incidentId]);
    ...
    if (error.message?.includes('incident_timeline_action_check')) {
      return res.status(500).json({
        error: 'Database constraint error: "severity_change" is not in the allowed actions. Run the migration: ALTER TABLE incident_timeline DROP CONSTRAINT incident_timeline_action_check; then re-add it with severity_change included.',
      });
    }

    res.status(500).json({ error: `Failed to update severity: ${error.message || 'Unknown error'}` });
  } finally {
    client.release();
  }
});
...
router.post('/:teamId/incidents/:incidentId/comments', async (req: Request, res: Response) => {
  try {
    const teamId = await resolveTeamId(req.params.teamId);
    if (!teamId) return res.status(404).json({ error: 'Team not found' });
...
router.get('/:teamId/incidents/:incidentId/timeline', async (req: Request, res: Response) => {
  try {
    const incidentId = req.params.incidentId as string;
    const result = await pool.query(
...
router.get('/:teamId/incidents/:incidentId/postmortem', async (req: Request, res: Response) => {
  try {
    const teamId = await resolveTeamId(req.params.teamId);
    if (!teamId) return res.status(404).json({ error: 'Team not found' });
    ...
    const ttaMs = timeToAck ? new Date(timeToAck.created_at).getTime() - createdAt.getTime() : null;
    const ttiMs = timeToInv ? new Date(timeToInv.created_at).getTime() - createdAt.getTime() : null;
    const ttrMs = timeToRes ? new Date(timeToRes.created_at).getTime() - createdAt.getTime() : null;
    ...
    const totalMs = resolvedAt ? resolvedAt.getTime() - createdAt.getTime() : Date.now() - createdAt.getTime();
    const sevChanges = timeline.filter((t: any) => t.action === 'severity_change');
    const comments = timeline.filter((t: any) => t.action === 'comment');
    const errorEvents = relatedEvents.filter((e: any) => e.severity === 'error' || e.severity === 'critical');
