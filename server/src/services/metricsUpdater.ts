...
export async function updateMetricsFromEvent(
  ...
  try {
    switch (event.source) {
      ...
      case 'sentry':
        if (['error', 'issue_triggered', 'issue_created'].includes(event.eventType)) {
          await recalculateErrorRate(teamId);
        }
        break;

      case 'uptime':
        if (event.eventType === 'ping_success' || event.eventType === 'ping_fail') {
          await updateUptimeMetric(teamId, event.eventType === 'ping_success');
        }
        if (event.eventType === 'latency_warning' || event.eventType === 'ping_success') {
...
async function updateUptimeMetric(teamId: string, isUp: boolean): Promise<void> {
...
async function updateResponseTime(teamId: string, responseTime: number): Promise<void> {
  await pool.query(`
    INSERT INTO metrics (team_id, type, value, timestamp)
